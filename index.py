...
# open file saved
#also for diff type of file saved?
            ...
            if os.stat('save.txt').st_size != 0:
                ...
                for tokens in nItems:
                    if tokens:
                        ...
                        item = genItem(title, price, link, priceTarget)
                        items.append(item)
                        ...
                        for el in priceArr:
                            ...
                            for el in priceArr:
                                if el is not None:
                                    ...
                                    break
        break
    item = genItem(title, convertedPrice, link, priceTarget)
    items.append(item)
    if len(title) > 20:
        title = title[:20]
...
#frame and cavas con tk library
#to change
...
inserLink = tk.Button(root, text="Insert Link", padx=10, pady=5,
                      fg="white", bg="#263D42", command=addGeneralLink)
